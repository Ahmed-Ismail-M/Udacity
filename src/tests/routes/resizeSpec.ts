  ...
  describe('test /api/resize with wrong parameters', () => {
    ...
    it('shoud return error width should be a number', (done) => {
      request(app)
        ...
        .expect(500)
        .expect('Error: Expected positive integer for width but received NaN of type number')
        .expect('Content-Type', 'text/html; charset=utf-8')
        .end((error:Error) => (error) ? done.fail(error) : done());
    });
    it('shoud return error height should be a number', (done) => {
      request(app)
        ...
        .expect(500)
        .expect('Error: Expected positive integer for height but received NaN of type number')
        .expect('Content-Type', 'text/html; charset=utf-8')
        .end((error:Error) => (error) ? done.fail(error) : done());
    });
